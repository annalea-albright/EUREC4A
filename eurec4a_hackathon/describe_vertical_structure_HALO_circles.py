...
def preprocess_circle_means(fp_flight, fp_dropsondes):
    ...
    circle_time_stamps = [(c["start"], c["end"]) for c in flightinfo["segments"] if c["kind"] == "circle"]
    ...
    subset_sondes = sondes_oneday.dropna(dim="launch_time", subset=['rh'], 
                                         how='any', thresh=300) 
    ...
    def calc_circle_mean_var(var, circle_time_stamps):
        ...
        for i in range(num_circles):
            ...
            var_circle_mean[i,:] = var.sel(launch_time=c_bool).mean(dim="launch_time", skipna=True)
    ...
    specific_humidity_circle_mean = calc_circle_mean_var(specific_humidity, circle_time_stamps)
    ...
    circle_means = xr.Dataset(
            data_vars={'mixing_ratio':    (('circle_num', 'alt'), mixing_ratio_circle_mean),
    ...
    def calc_density(ds, pres_str, temp_str, mixing_ratio_str):
        ...
        if (ds[pres_str].max().values < 1200) : pressure = ds[pres_str]*100
        else: pressure = ds[pres_str]
        # convert temperature from Celsius to Kelvin
        if (ds[temp_str].max().values < 100) : temp_K = ds[temp_str] + 273.15 
        else: temp_K = ds[temp_str]
        # convert mixing ratio from g/kg to kg/kg
        if (ds[mixing_ratio_str].max().values > 10) : mixing_ratio = ds[mixing_ratio_str] / 1000
        else: mixing_ratio = ds[mixing_ratio_str]
        
        Rd = typhon.constants.gas_constant_dry_air          # J/kg/K    
        Rv = typhon.constants.gas_constant_water_vapor    
        eps = Rd / Rv
        ds = ds.assign(density = ( pressure ) / (Rd * (temp_K ) * \
           (1 + (mixing_ratio/eps))/(1 + mixing_ratio)))
    ...
    def calc_peak_RH_linearize(ds, rh_str):
        ...
        mixed_layer = np.logical_and(alt_vec >= lower_bound_mixed_layer, alt_vec <= upper_bound_mixed_layer) 
        lower_ml = np.logical_and(alt_vec >= 200, alt_vec <= 400) # for linearized RH profile
        ...
        for i in range(len(ds.circle_num)):
            ...
            model = sm.OLS(rh_profile[lower_ml].values,X).fit() # instantiate linear model 
            ...
            idx_peaks_RH, _ = find_peaks(rh_profile[mixed_layer]) # returns indices of local maxima
            ...
            RH_peaks = rh_profile[idx_peaks_RH + add_to_index_ml_lb] # RH values at the local maxima
            RH_extrapolated = predicted_RH_line[idx_peaks_RH + add_to_index_ml_lb] # extrapolated RH values at the observed, local RH maxima
            difference_peak_extrap = min(abs(RH_peaks - RH_extrapolated)) # find minimum horizontal difference (RH peak - RH extrapolated)
            
            alt_peakRH[i] = difference_peak_extrap.alt # altitude where distance is minimized
            val_peakRH[i] = rh_profile.values[int(alt_peakRH[i]/10)] # RH value at this altitude, index from height / 10
            
        # plot?
        if False: 
            
            # if RH units [0,1] convert to [0,100] for plotting
            if (ds[rh_str].max().values < 10) : RH = ds[rh_str] *100
            else: RH = ds[rh_str]
            
            plt.figure(figsize=(8,10))  
            colors = ["navy", "deepskyblue", "crimson", "mediumaquamarine", "darkmagenta", "goldenrod"]
            for i in range(len(ds.circle_num)):
                ...
                plt.plot(rh_profile[first_900_m], alt_vec[first_900_m],color=colors[i], alpha=0.6, linewidth=4)  
                ...
                predicted_RH_line = model.predict(sm.add_constant(alt_vec[first_900_m]))
                plt.plot(predicted_RH_line, alt_vec[first_900_m],color=colors[i], alpha=0.7, linewidth=3)
    ...
    def find_ml_height_from_gradient(ds, var_str, threshold):
        ...
        def calculateHmix_var(density_profile, var_profile, threshold):
            ...
            while(abs(var_diff) < threshold):
                ...
                numer += 0.5*(density_profile[k+1] * var_profile[k+1] + density_profile[k] * var_profile[k])
    ...
    circle_means = find_ml_height_from_gradient(circle_means, 'mixing_ratio', threshold=0.3 / 1000)
    # dthetav/dz
    ...
    def find_LCL_Bolton(ds, temp_str, rh_str, levels):
        ...
        if (ds[rh_str].max().values > 2) : RH = ds[rh_str] / 100                     
        else:   RH = ds[rh_str]
        nt = len(ds.circle_num) # number of circles
        LCL_Bolton = np.zeros([nt])
        for i in range(nt):
            LCL_Bolton[i] = calc_LCL_Bolton(temp_K[i,:] , RH[i,:], ds['alt'], levels=levels)
    ...
    def calc_static_stability(ds, pres_str, temp_str):
        ...
        # convert Pa to hPa
        if (ds[pres_str].max().values > 1400) : ds[pres_str] = ds[pres_str] / 100 
        
        ds['static_stability'] = - (temp_K / ds["theta"]) * (ds["theta"].diff(dim="alt")/ds[pres_str].diff(dim="alt"))
    ...
    def find_inversion_height_static_stability(ds, max_alt):
        ...
        window = 10 # window for convolution
        static_stability_smoothed = static_stability.rolling(alt=window, min_periods = window, center=True).mean(skipna=True)[:,10:]
        ...
        inversion_height_stability = calc_inversion_height_static_stability(static_stability, smooth=False)
        inversion_height_stability_smoothed = calc_inversion_height_static_stability(static_stability_smoothed, smooth=True)
        
        # assign to xarray
        ds = ds.assign(inversion_height_stability =  inversion_height_stability)
        ds = ds.assign(inversion_height_stability_smoothed =  inversion_height_stability_smoothed)
        
        return ds
    
    # call outer function
    circle_means = find_inversion_height_static_stability(circle_means, max_alt=max_alt_inv)
    ...
    def find_inversion_height_grad_RH(ds, max_alt):
        ...
        window = 10 # window for convolution
        RH_smooth = ds['RH'].rolling(alt=window, min_periods = window, center=True).mean(skipna=True)
        RH = ds['RH']
        ...
        def calc_inversion_height_grad_RH(RH):
            ...
            for i in range(len(RH)):
                ...
                lower_tropo = np.logical_and(circle_means.alt > 1500, circle_means.alt < max_alt)
                inversion_ind = np.argmax(gradient_RH[lower_tropo]) # index of first maximum in this domain
        ...
        ds = ds.assign(inversion_height_grad_RH_smoothed =  inversion_height_grad_RH_smoothed)
    ...
    def find_inversion_height_grad_RH_and_T(ds, max_alt):
        ...
        for i in range(num_iters):
            ...
            idx_base = np.argmax(inversion_levels.isel(circle_num=i)<3000) # first maximum below 3 km
        ...
        if 0:
            ...
            for i in range(len(gradient_RH)):
                ax[0].plot(gradient_RH.isel(circle_num=i), gradient_RH.alt.values,color="lightgrey", linewidth=2,alpha=0.5)
            ax[0].plot(gradient_RH.mean(dim='circle_num'),gradient_RH.alt.values,linewidth=4, color='black')
            ...
            for i in range(len(gradient_T)):
                ax[1].plot(gradient_T.isel(circle_num=i), gradient_T.alt.values,color="lightgrey", linewidth=2,alpha=0.5)
            ax[1].plot(gradient_T.mean(dim='circle_num'),gradient_T.alt.values,linewidth=4, color='black')
            ...
            plt.plot(range(len(inv_base_height_gradients)), inv_base_height_gradients, linewidth=4, color='black')
            plt.xlim([range(len(inv_base_height_gradients))[0], range(len(inv_base_height_gradients))[-1]])
            #plt.xlim([x1, x2])
            plt.gca().spines['right'].set_visible(False)
            plt.gca().spines['top'].set_visible(False)
    ...
    # call function
    circle_means = find_inversion_height_grad_RH_and_T(circle_means, max_alt=max_alt_inv)
    ...
    if False:
        ...
        plt.plot(range(len(circle_means.circle_num)), circle_means.inversion_height_stability, linewidth=4, color='grey', label ='static stability criterion')
        plt.plot(range(len(circle_means.circle_num)), circle_means.inversion_height_stability_smoothed, linewidth=4, color='midnightblue', label ='static stability smoothed')
        plt.plot(range(len(circle_means.circle_num)), circle_means.inversion_height_grad_RH, linewidth=4, color='blue', label ='RH')
        plt.plot(range(len(circle_means.circle_num)), circle_means.inversion_height_grad_RH_T, linewidth=4, color='red', label ='RH + temp criterion')
    ...
    def find_lower_tropospheric_stability(ds):
        ...
        def calc_lts(ds):
            ...
            # convert Pa to hPa
            if (ds['pressure'].max().values > 1400) : pressure = ds['pressure'] / 100 
            else: pressure = ds['pressure']
            ...
            for i in range(num_iters):
                ...
                pot_temp_interp = interp1d(iter_pres, iter_theta, fill_value='extrapolate', bounds_error=False)
    ...
    def find_eis(ds):
        ...
        def calc_eis(ds):
            ...
            # Celsius to Kelvin
            if (ds['temp'].max().values < 100) : temp_K = ds['temp'] + 273.15 
            else: temp_K = ds['temp'] 
            # convert hPa to Pa
            if (ds['pressure'].max().values < 1400) : pressure_Pa = ds['pressure'] * 100 
            else: pressure_Pa = ds['pressure']
            ...
            for i in range(num_iters):
                ...
                moist_lapse_rate = typhon.physics.moist_lapse_rate(pressure_Pa[i,:], temp_K[i,:])
                moist_lapse_rate_850hPa = interp1d(pressure_Pa[i,:], moist_lapse_rate)(850e2)
    ...
    def layer_mean_wind_speed_dir(ds):
        ...
        for i in range(num_iters):
            mixed_layer_ind = np.logical_and(altitude >= 10, altitude <= mixed_layer_height[i].values)
            ...
            cloud_layer_ind = np.logical_and(altitude >= LCL[i].values, altitude <= inversion_base[i].values)
    ...
    def plot_all_sondes(circle_means, var, xlabel, xlim1, xlim2):
        ...
        for i in range(len(circle_means.circle_num)):
            plt.plot(var.isel(circle_num=i), var.alt.values,color="lightgrey", linewidth=2,alpha=1)
    ...
    # =============================================================================  
    if False:
        plot_all_sondes(circle_means, circle_means.mixing_ratio * 1000, xlabel='Mixing ratio / g/kg ', xlim1 =0, xlim2=20)
        plot_all_sondes(circle_means, circle_means.theta, xlabel='theta / K', xlim1 =295, xlim2=320)
        plot_all_sondes(circle_means, circle_means.theta_v, xlabel='theta_v / K', xlim1 =297, xlim2=320)
        plot_all_sondes(circle_means, circle_means.temp, xlabel='temperature / K', xlim1 =5, xlim2=30)
        plot_all_sondes(circle_means, circle_means.RH, xlabel='RH / %', xlim1 =0, xlim2=100)
        plot_all_sondes(circle_means, circle_means.wspd, xlabel='wind speed / m/s', xlim1 =0, xlim2=15)

        # different estimates of mixed layer heights
        plt.figure(figsize=(15,10))
        plt.plot(circle_means['mixed_layer_height_RHmax_2'].values, linewidth=3, label='RHmax')  
        plt.plot(circle_means['mixed_layer_height_grad_theta_v'].values, linewidth= 3, label='thetav')    
        plt.plot(circle_means['mixed_layer_height_grad_mixing_ratio'].values, linewidth=3, label='mixing ratio')    
        plt.legend()
        plt.title(day_str)
        ...
        for i in range(len(circle_means[rh_str])):
            ...
            if (circle_means[rh_str].max().values < 10):
                plt.plot(circle_means[rh_str][i,:]*100, alt_vec,color=next(colors), alpha=0.5, linewidth=4)
            else:
                plt.plot(circle_means[rh_str][i,:], alt_vec,color=next(colors), alpha=0.5, linewidth=4)  
        plt.gca().spines['right'].set_visible(False)
        plt.gca().spines['top'].set_visible(False)
        plt.ylabel('Altitude / m')
        plt.xlabel('RH (%)')
        ...
        plt.figure(figsize=(12,8))
        plt.plot(range(len(circle_means[rh_str])), circle_means['mixed_layer_height_RHmax'], linewidth=4, color='black')
        #plt.xlim([x1, x2])
        plt.gca().spines['right'].set_visible(False)
        plt.gca().spines['top'].set_visible(False)
        ...
        sns.set(context='notebook', style='white', palette='deep', font='sans-serif', font_scale=1.8, color_codes=True, rc=None)
        sns.distplot(circle_means['LCL_Bolton'], bins=20, norm_hist=True,hist=False, kde=True, vertical=True,\
                         kde_kws={"color": "royalblue", "lw": 3, "label": f"HALO LCL= {int(np.ceil(circle_means.LCL_Bolton.mean().values))} $\pm$ {int(np.ceil(circle_means.LCL_Bolton.std().values))} ({len(circle_means.LCL_Bolton)} sondes)"},
...
def load_flight_files(fp_flights):
    ...
    for filename in sorted(glob.glob(os.path.join(fp_flights, '*.yaml'))):
      with open(filename, 'r') as f:
        ...
        count += 1
    return d
fp_flights = os.path.join(input_dir,'HALO_flight_partitioning') # folder with individual partitioning files
...
circle_means_RF14 = circle_means_RF14_raw.dropna(dim="circle_num",subset=["temp"], how="any", thresh=100)
...
def load_data_circles(var_str):
    ...
    flight_ids = ['02', '03', '04', '05', '07', '08', '09','10', '11', '12', '13', '14']
    ...
    var_rs[-1,2] = np.nan # third circle in last flight not performed within HALO circle (by NTAS buoy)
    return var_rs
...
mixed_layer_height_RHmax = load_data_circles(var_str = 'mixed_layer_height_RHmax_2') # or, mixed_layer_height_RHmax
mixed_layer_height_grad_mixing_ratio = load_data_circles(var_str = 'mixed_layer_height_grad_mixing_ratio')
mixed_layer_height_grad_theta_v = load_data_circles(var_str = 'mixed_layer_height_grad_theta_v')
...
sns.set(context='notebook', style='whitegrid', palette='deep', font='sans-serif', font_scale=2, color_codes=True, rc=None)
g = sns.jointplot(x=mixed_layer_height_RHmax, y=mixed_layer_height_grad_theta_v, kind="kde", color="m", height=8, xlim=(320, 880), ylim=(320, 880))  #                   marginal_kws=dict(bw=40),bw=40
g.ax_joint.collections[0].set_alpha(0)
g.set_axis_labels("relative humidity maximum", "$\partial\Theta_v/\partial z$ = 0 $\mid$ threshold");
...
g = sns.jointplot(x=mixed_layer_height_RHmax, y=mixed_layer_height_grad_mixing_ratio, kind="kde", color="m", height=8, xlim=(320, 880), ylim=(320, 880)) 
g.ax_joint.collections[0].set_alpha(0)
g.set_axis_labels("relative humidity maximum", "$\partial q/\partial z$ = 0 $\mid$ threshold");
x0, x1 = g.ax_joint.get_xlim()
y0, y1 = g.ax_joint.get_ylim()
lims = [max(x0, y0), min(x1, y1)]
lims = [max(x0, y0), min(x1, y1)]
...
g = sns.jointplot(x=mixed_layer_height_grad_theta_v, y=mixed_layer_height_grad_mixing_ratio, kind="kde", color="m", height=8) 
g.ax_joint.collections[0].set_alpha(0)
g.set_axis_labels("$\partial\Theta_v/\partial z$ = 0 $\mid$ threshold", "$\partial q/\partial z$ = 0 $\mid$ threshold");
x0, x1 = g.ax_joint.get_xlim()
y0, y1 = g.ax_joint.get_ylim()
lims = [max(x0, y0), min(x1, y1)]
lims = [max(x0, y0), min(x1, y1)]
...
plt.figure(figsize=(10,10)); sns.distplot(mixed_layer_height_RHmax-mixed_layer_height_grad_mixing_ratio); plt.xlim([-200,200]); plt.title('RHmax - dq/dz')
plt.figure(figsize=(10,10)); sns.distplot(mixed_layer_height_RHmax-mixed_layer_height_grad_theta_v); plt.xlim([-200,200]); plt.title('RHmax - dthetav/dz')
plt.figure(figsize=(10,10)); sns.distplot(mixed_layer_height_grad_mixing_ratio-mixed_layer_height_grad_theta_v); plt.xlim([-200,200]); plt.title('dq/dz - dthetav/dz')
...
residuals_thetav_q = mixed_layer_height_grad_theta_v -mixed_layer_height_grad_mixing_ratio
...
listofDates = ['2020-01-22','2020-01-24', '2020-01-26', '2020-01-28', '2020-01-31', '2020-02-02',\
    '2020-02-05', '2020-02-07', '2020-02-09', '2020-02-11', '2020-02-13', '2020-02-15']

days = ['01-22','01-24', '01-26', '01-28', '01-31', '02-02',\
         '02-05', '02-07', '02-09', '02-11', '02-13', '02-15']

flights = ["RF02_01_22", "RF03_01_24", "RF04_01_26", "RF05_01_28", "RF07_01_31", "RF08_02_02", \
           "RF09_02_05", "RF10_02_07", "RF11_02_09", "RF12_02_11", "RF13_02_13", "RF14_02_15"]

# research flights short-hand
rfs = ["RF02", "RF03", "RF04", "RF05", "RF07", "RF08", \
           "RF09", "RF10", "RF11", "RF12", "RF13", "RF14"]
...
research_flight = np.hstack((research_flight, research_flight_12, research_flight13, research_flight14))
